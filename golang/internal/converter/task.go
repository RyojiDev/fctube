...
func (vc *VideoConverter) processVideo(task *VideoTask) error {
	...
	if err != nil {
		vc.logError(*task, "failed to merge chunks %v", err)
		return fmt.Errorf("failed to merge chuncks %v", err)
	}
	...
	slog.Info("Video converted to mpeg-dash", slog.String("path", task.Path))
